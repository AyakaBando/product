...
if( $_GET['mode'] == 1 )
{
    ...
    $endPoint = $query->GetCulumnContents( 'product_catalog', 'endPoint', 'imgId', $_GET['imgId'], 1 );
...
if( $_GET['mode'] == 2 )
{
    ...
    $endPoint = $query->GetCulumnContents( 'product_drawing', 'endPoint', 'imgId', $_GET['imgId'], 1 );
    $dwFileName = $query->GetCulumnContents( 'product_drawing', 'dwFileName', 'imgId', $_GET['imgId'], 1 );
    $genre = 'drawing';
}

if ($_SESSION['member']['eMail'] && $endPoint) {
    $name = $_GET['name'];
    ...
    $contents = file_get_contents($endPoint, false, stream_context_create($options));
}
